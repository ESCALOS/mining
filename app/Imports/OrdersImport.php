    ...
    public function onRow(Row $row)
    {
        ...
        if($row['ticket'] != "" && $row['ticket'] != null && $tipoDocumento != "" && Order::where('batch',$row['lote_orden'])->doesntExist()) {
            ...
            DB::transaction(function() use($order,$row){
                ...
                $payableTotal->unit_price_copper =floor((($row['cobre_internacional'] - $row['proteccion_cobre'])*2204.62)*100)/100;
                $payableTotal->total_price_copper =floor($payableTotal->unit_price_copper*$payableTotalCopperPercent*1000)/1000;

                $payableTotalSilverPercent = floor(($row['ley_plata']*$row['factor_plata']*$row['pagable_plata']/100-$row['deduccion_plata'])*1000)/1000;
                ...
                $payableTotalGoldPercent = floor(($row['ley_oro']*$row['factor_plata']*$row['pagable_oro']/100-$row['deduccion_oro'])*100)/100;
                $payableTotal->unit_price_gold =floor(($row['oro_internacional'] - $row['proteccion_oro'])*100)/100;
