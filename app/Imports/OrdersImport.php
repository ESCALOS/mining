    ...
    public function onRow(Row $row)
    {
        ...
        if($row['ticket'] != "" && $row['ticket'] != null && $tipoDocumento != "" && Order::where('batch',$row['lote_orden'])->doesntExist()) {
            ...
            DB::transaction(function() use($order,$row){
                ...
                $payableTotalSilverPercent = floor(((floor($row['ley_plata']*$row['factor_plata']*1000)/1000)*$row['pagable_plata']/100-$row['deduccion_plata'])*100)/100;
                ...
                $payableTotalGoldPercent = floor(((floor($row['ley_oro']*$row['factor_oro']*1000)/1000)*$row['pagable_oro']/100-$row['deduccion_oro'])*100)/100;
